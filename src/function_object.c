...
/**
 * Returns the first row index at which a given one-dimensional array of types
 * can be found in a two-dimensional reference array of types (or `-1` if not
 * found).
 *
 * ## Notes
 *
 * -   The intended use case for this function is for type dispatch (i.e., given
 *     a set of array data types, find a matching interface according the
 *     interface's accepted array data types).
 * -   The function assumes that `X` is stored in row-major order.
 * -   The function assumes that the number of indexed elements in `Y` equals
 *     the number of columns in `X`.
 * -   The function returns a row index. To convert to a linear index, multiply
 *     `strideX1` by the return value.
 *
 * @private
 ...
 * int64_t r1 = ndarray_function_dispatch_types_index_of(N, M, X, M, 1, Y1, 1);
 * // returns 1
 *
 * // Define a search array:
 * int32_t Y2[] = {
 ...
 *
 * // Find the list of types:
 * int64_t r2 = ndarray_function_dispatch_types_index_of(N, M, X, M, 1, Y2, 1);
 * // returns -1
 */
static int64_t ndarray_function_dispatch_types_index_of(
    const int64_t N, const int64_t M, const int32_t* X, const int64_t strideX1,
    const int64_t strideX2, const int32_t* Y, const int64_t strideY
) {
  int64_t ox;
  int64_t oy;
  int64_t ix;
  ...
  for (i = 0; i < N; i++) {
    ...
    // If we successfully iterated over all columns, then that means we've found
    // a match...
    if (j == M) {
      return i;
    }
 ...
 * @param types       array of type "numbers", where the total number of types
 *                    equals `(nin+nout)*nfunctions` and where each set of
 *                    `nin+nout` consecutive types (non-overlapping) corresponds
 *                    to the set of ndarray argument types for a corresponding
 *                    ndarray function
 * @param data        array of void pointers corresponding to the "data" (e.g.,
 *                    callbacks) which should be passed to a respective ndarray
 *                    function
 * @return            pointer to a dynamically allocated ndarray function object
 *                    or, if unable to allocate memory, a null pointer
 *
 * @example
 * #include "ndarray/base/function_object.h"
 * #include "ndarray/base/unary.h"
 * #include "ndarray/dtypes.h"
 * #include <stdlib.h>
 * #include <stdio.h>
 *
 ...
 * // assume that the output ndarray is always a double-precision floating-point
 * // number array):
 ...
 * // Create a new ndarray function object:
 * struct ndarrayFunctionObject *obj =
 *     ndarray_function_allocate(name, 1, 1, functions, 2, types, data);
 * if (obj == NULL) {
 *     fprintf(stderr, "Error allocating memory.\n");
 *     exit(1);
 * }
 ...
 * int64_t idx = ndarray_function_dispatch_index_of(obj, itypes);
 * if (idx < 0) {
...
int64_t ndarray_function_dispatch_index_of(
  ...
  return ndarray_function_dispatch_types_index_of(
      (int64_t)N, M, obj->types, M, 1, types, 1
  );
}
